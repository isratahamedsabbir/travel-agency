    ...
    <div class="sl-mainpanel">
      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">Searching, ticketing and paging goodness will be immediately added to the table, as shown in this example.</p>
            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($ticket as $ticket_key => $ticket_value)
                <tr class="{{ $ticket_value->new == 1 ? 'bg-danger' : '' }}">
                  <td>{{ $sn++ }}</td>
                  <td class="text-dark">{{ $ticket_value->name }}</td>
                  <td class="text-dark">{{ text_shorten($ticket_value->email, 30) }}</td>
                  <td class="text-dark">{{ $ticket_value->mobile }}</td>
                  <td class="text-dark">{{ naturalDate($ticket_value->date) }}</td>
					...
					<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item {{ $ticket_value->new == 0 ? 'd-none' : '' }}" href="{{ url('/dashboard/air_tickets/conform/'.$ticket_value->id) }}">Conform</a>
						  <a class="dropdown-item" href="{{ url('/dashboard/air_tickets/delete/'.$ticket_value->id) }}">Delete</a>
						  <button class="dropdown-item" data-toggle="modal" data-target="#modaldemo{{$ticket_value->id}}">Open</button>
						</div>
					  </div>
					</div>
				  </td>
                </tr>
				{{ ticket_details($ticket_value) }}
				@empty
				<tr>
