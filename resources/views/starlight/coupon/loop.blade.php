            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($service as $service_key => $service_value)
                <tr>
                  <td>{{ $sn++ }}</td>
                  <td>{{ text_shorten($service_value->name, 50) }}</td>
                  <td>{{ text_shorten($service_value->title, 50) }}</td>
				    ...
				    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="{{ url('dashboard/service/update/page/'.$service_value->id) }}">Update</a>
						  <a class="dropdown-item" href="{{ url('dashboard/service/remove/'.$service_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
