    ...
    <div class="sl-mainpanel">
      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">Searching, hoteling and paging goodness will be immediately added to the table, as shown in this example.</p>
            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($hotel as $hotel_key => $hotel_value)
                <tr class="{{ $hotel_value->new == 1 ? 'bg-danger' : '' }}">
                  <td>{{ $sn++ }}</td>
                  <td class="text-dark">{{ $hotel_value->name }}</td>
                  <td class="text-dark">{{ text_shorten($hotel_value->email, 30) }}</td>
                  <td class="text-dark">{{ $hotel_value->phone }}</td>
                  <td class="text-dark">{{ naturalDate($hotel_value->date) }}</td>
					...
					<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item {{ $hotel_value->new == 0 ? 'd-none' : '' }}" href="{{ url('/dashboard/hotel/conform/'.$hotel_value->id) }}">Conform</a>
						  <a class="dropdown-item" href="{{ url('/dashboard/hotel/delete/'.$hotel_value->id) }}">Delete</a>
						  <button class="dropdown-item" data-toggle="modal" data-target="#modaldemo{{$hotel_value->id}}">Open</button>
						</div>
					  </div>
					</div>
				  </td>
                </tr>
				{{ hotel_details($hotel_value) }}
				@empty
				<tr>
