    ...
    <div class="sl-mainpanel">
      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">Searching, subscribering and paging goodness will be immediately added to the table, as shown in this example.</p>
            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($subscriber as $subscriber_key => $subscriber_value)
                <tr class="{{ $subscriber_value->new == 1 ? 'bg-danger' : '' }}">
                  <td>{{ $sn++ }}</td>
                  <td class="text-dark">{{ $subscriber_value->email }}</td>
                  <td class="text-dark">{{ naturalDate($subscriber_value->date) }}</td>
					...
					<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item {{ $subscriber_value->new == 0 ? 'd-none' : '' }}" href="{{ url('/dashboard/subscriber/conform/'.$subscriber_value->id) }}">Conform</a>
						  <a class="dropdown-item" href="{{ url('/dashboard/subscriber/delete/'.$subscriber_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
