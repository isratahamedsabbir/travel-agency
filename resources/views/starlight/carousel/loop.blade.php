            ...
            <table id="datatable1" class="table display responsive nowrap">
              <thead>
                <tr>
                  ...
                  <th class="wd-15p text-center">picture</th>
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($carousel as $carousel_key => $carousel_value)
                <tr>
                  <td>{{ $sn++ }}</td>
                  <td>{{ text_shorten($carousel_value->title, 50) }}</td>
                  <td class="text-center">
                    <a href="{{ asset('uploads/'.$carousel_value->picture) }}" >
                      @if(str_contains($carousel_value->picture, '.mp4'))
                        MP4
                      @else
                        <img class="img-fluid" src="{{ url('uploads/'.$carousel_value->picture) }}" style="width:30px;height:30px;"/>
                      @endif
                    </a>
                  </td>
                  <td class="text-center">
				    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="{{ url('dashboard/carousel/update/page/'.$carousel_value->id) }}">Update</a>
						  <a class="dropdown-item" href="{{ url('dashboard/carousel/remove/'.$carousel_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
