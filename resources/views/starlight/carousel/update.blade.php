      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">A form with a label on top of each form control.</p>
			<form action="{{ url('dashboard/carousel/update/function/'.$carousel->id) }}" method="post" enctype="multipart/form-data">
			  @csrf
				...
				<div class="row mg-b-25">
				  <div class="col-lg-12">
					<div class="form-group">
					  ...
					  <input class="form-control" type="text" name="title" value="{{ $carousel->title }}">
					</div>
				  </div><!-- col-12 -->
				  <div class="col-lg-7">
					<div class="form-group">
            ...
            @if(str_contains($carousel->picture, '.mp4'))
              <video class="img-fluid" autoplay loop muted>
                <source src="{{ url('uploads/'.$carousel->picture) }}" type="video/mp4" />
              </video>
            @else
              <img class="img-fluid" src="{{ url('uploads/'.$carousel->picture) }}" />
            @endif
          </div>
				  </div><!-- col-5 -->
				</div><!-- row -->
