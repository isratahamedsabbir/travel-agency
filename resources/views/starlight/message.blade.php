    ...
    <div class="sl-mainpanel">
      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">Searching, messageing and paging goodness will be immediately added to the table, as shown in this example.</p>
            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($message as $message_key => $message_value)
                <tr class="{{ $message_value->new == 1 ? 'bg-danger' : '' }}">
                  <td>{{ $sn++ }}</td>
                  <td class="text-dark">{{ $message_value->name }}</td>
                  <td class="text-dark">{{ $message_value->email }}</td>
                  <td class="text-dark">{{ $message_value->mobile }}</td>
                  <td class="text-dark">{{ naturalDate($message_value->date) }}</td>
					...
					<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item {{ $message_value->new == 0 ? 'd-none' : '' }}" href="{{ url('/dashboard/message/conform/'.$message_value->id) }}">Read</a>
						  <a class="dropdown-item" href="mailto:{{ $message_value->email }}?subject={{ text_shorten($message_value->message, 30) }}&body=hello Sir, ">Send Email</a>
						  <a class="dropdown-item" href="{{ url('/dashboard/message/delete/'.$message_value->id) }}">Delete</a>
						  <button class="dropdown-item" data-toggle="modal" data-target="#modaldemo{{$message_value->id}}">Open</button>
						</div>
					  </div>
					</div>
				  </td>
                </tr>
				{{ message_details($message_value) }}
				@empty
				<tr>
