            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($membership as $membership_key => $membership_value)
                <tr>
                  <td>{{ $sn++ }}</td>
                  <td class="text-center"><a href="{{ asset('uploads/'.$membership_value->image) }}" ><img src="{{ asset('uploads/'.$membership_value->image) }}" style="width:30px;height:30px;"/></a></td>
                  <td>{{ text_shorten($membership_value->title, 50) }}</td>
                  <td>{{ $membership_value->code }}</td>
				    ...
				    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="{{ url('dashboard/membership/update/page/'.$membership_value->id) }}">Update</a>
						  <a class="dropdown-item" href="{{ url('dashboard/membership/remove/'.$membership_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
