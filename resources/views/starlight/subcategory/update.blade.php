      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-40">
          ...
          <p class="mg-b-20 mg-sm-b-30">A form with a label on top of each form control.</p>
			<form action="{{ url('dashboard/subcategory/update/function/'.$subcategory->id) }}" method="post" enctype="multipart/form-data">
			  @csrf
          ...
          <select type="text" id="title" class="form-control" name="category" data-parsley-trigger="change" required="">
            ...
            @forelse($category as $category_key => $category_value)
              <option value="{{ $category_value->id }}" {{ $subcategory->category == $category_value->id ? 'selected' : '' }} > {{ $category_value->name }} </option>
              @empty
            ...
            @endforelse
				  ...
				  <div class="col-lg-7">
					    <label class="icon" class="form-label">Icon: <span class="tx-danger">*</span></label>
					    <input class="form-control" type="file" name="icon">
					    <small>Please Imput jpg,jpeg,png,bmp file.</small>
				  </div><!-- col-7 -->
				  <div class="col-lg-5">
					  <div class="form-label text-center">
					    <a href="{{ url('uploads/'.$subcategory->icon) }}" ><img src="{{ asset('uploads/'.$subcategory->icon) }}" style="width:120px;" alt="{{ $subcategory->icon }}"/></a>
					  </div>
				  </div><!-- col-5 -->
				</div><!-- row -->
