            ...
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($subcategory as $subcategory_key => $subcategory_value)
                <tr>
                  <td>{{ $sn++ }}</td>
                  <td>{{ $subcategory_value->name }}</td>
                  <td>{{ App\Models\Category::find($subcategory_value->category)->name }}</td>
                  <td class="text-center"><a href="{{ asset('uploads/'.$subcategory_value->icon) }}" ><img src="{{ asset('uploads/'.$subcategory_value->icon) }}" style="width:30px;height:30px;"/></a></td>
				    ...
				    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="{{ url('dashboard/subcategory/update/page/'.$subcategory_value->id) }}">Update</a>
						  <a class="dropdown-item" href="{{ url('dashboard/subcategory/remove/'.$subcategory_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
