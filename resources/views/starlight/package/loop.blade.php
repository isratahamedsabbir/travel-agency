            ...
            <table id="datatable1" class="table display responsive nowrap">
              <thead>
                <tr>
                  ...
                  <th class="wd-15p text-center">icon</th>
              ...
              <tbody>
				@php
					$sn=1
				@endphp
				@forelse($package as $package_key => $package_value)
                <tr>
                  <td>{{ $sn++ }}</td>
                  <td>{{ text_shorten($package_value->title, 50) }}</td>
                  <td>{{ App\Models\Category::find($package_value->category)->name }}</td>
                  <td>{{ App\Models\Subcategory::find($package_value->subcategory)->name }}</td>
                  <td class="text-center"><a href="{{ asset('uploads/'.$package_value->thumb) }}" ><img src="{{ asset('uploads/'.$package_value->thumb) }}" style="width:30px;height:30px;"/></a></td>
				    ...
				    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					  <div class="btn-group" role="group">
						...
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="{{ url('dashboard/package/update/page/'.$package_value->id) }}">Update</a>
						  <a class="dropdown-item" href="{{ url('dashboard/package/remove/'.$package_value->id) }}">Delete</a>
						</div>
					  </div>
					</div>
				  </td>
